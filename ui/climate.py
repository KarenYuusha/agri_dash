...
def show_climate_page():
    ...
    st.markdown("Khám phá mối liên hệ giữa các chỉ số khí tượng và điều kiện môi trường.")

    @st.cache_data
    def load_climate_data():
        try:
            df = pd.read_csv("raw_data/nasa_power_daily.csv")
            # Chuyển đổi Date từ '20080101' sang Datetime
            df['Date'] = pd.to_datetime(df['Date'].astype(str), format='%Y%m%d')
            df['Year'] = df['Date'].dt.year
            df['Month'] = df['Date'].dt.month
            df['Province'] = df['Province'].str.strip()
            # Đảm bảo các cột giá trị là số
            numeric_cols = ["T2M", "RH2M", "T2M_MAX", "T2M_MIN", "CLOUD_AMT", "PRECTOTCORR", "GWETPROF", "CLRSKY_SFC_SW_DWN"]
    ...
    if df_cli is not None:
        ...
        with st.sidebar:
            st.header("📍 Cấu hình")
            sel_prov = st.selectbox("Chọn tỉnh thành:", sorted(df_cli['Province'].unique()), key="c_prov")
            
            min_y, max_y = int(df_cli['Year'].min()), int(df_cli['Year'].max())
            yr_range = st.slider("Khoảng năm:", min_y, max_y, (min_y, max_y), key="c_yrs")
            
            # User chooses which variables to include in the analysis
        ...
        if df_f.empty:
            ...
            # --- 1. TREND ANALYSIS (Monthly Average) ---
            st.subheader(f"Xu hướng biến thiên tại {sel_prov}")
            df_m = df_f.groupby(['Year', 'Month'])[sel_params].mean().reset_index()
            df_m['Date_Display'] = pd.to_datetime(df_m[['Year', 'Month']].assign(DAY=1)[['Year', 'Month', 'DAY']])
            ...
            st.subheader("🔗 Ma trận Tương quan Toàn diện")
            st.markdown("""
            ...
            fig_corr = px.imshow(
                ...
                aspect="auto",
                color_continuous_scale='RdBu_r', # Red-Blue scale (Red is negative, Blue is positive)
                range_color=[-1, 1],
            ...
            with st.expander("💡 Giải thích kết quả tương quan"):
                ...
                """)
                st.dataframe(df_f[sel_params].describe().T, use_container_width=True)