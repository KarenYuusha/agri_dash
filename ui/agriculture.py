...
def show_agriculture_page():
    ...
    with tab1:
        ...
        with c1:
            lev_map = {"Toàn quốc": "National",
                       "Vùng": "Regional", "Tỉnh": "Province"}
            s_lev_label = st.selectbox(
                "Cấp độ", list(lev_map.keys()), key="q_lev")
            s_lev_val = lev_map[s_lev_label]
        with c2:
            reg_opts = sorted(df[df['Level'] == s_lev_val]
                              ['Province'].unique())
        ...
        with c4:
            ...
            s_comm = st.selectbox(
                "Nông sản (để xem Metric)", comm_opts, key="q_comm")
    ...
    with tab2:
        ...
        with st.container(border=True):
            ...
            c_sel = st.radio("**So sánh theo (Màu sắc biểu đồ):**",
                             list(comp_map.keys()), horizontal=True, key="t_comp")
            color_col = comp_map[c_sel]

        f1, f2, f3 = st.columns(3)
        with f1:
            t_lev_map = {"Toàn quốc": "National",
                         "Vùng": "Regional", "Tỉnh": "Province"}
            t_lev_val = t_lev_map[st.selectbox(
                "Cấp độ lọc:", list(t_lev_map.keys()), key="t_lev")]
            t_space_opts = sorted(
                df[df['Level'] == t_lev_val]['Province'].unique())
            t_spaces = st.multiselect(
                "Chọn địa phương:", t_space_opts, default=t_space_opts[:3], key="t_spaces")
        with f2:
            t_comms = st.multiselect("Chọn loại nông sản:", sorted(
                df['Commodity'].unique()), default=df['Commodity'].unique()[:3], key="t_comms")
        with f3:
            t_attr = st.selectbox("Chọn chỉ số:", sorted(
                df['Attribute'].unique()), key="t_attr")
            t_yrs = st.slider("Khoảng năm:", int(df['Year'].min()), int(
                df['Year'].max()), (2008, 2023), key="t_yrs")

        # Filtering
        df_t = df[(df['Year'].between(t_yrs[0], t_yrs[1])) & (
            df['Level'] == t_lev_val) & (df['Attribute'] == t_attr)]
        if t_spaces:
            df_t = df_t[df_t['Province'].isin(t_spaces)]
        if t_comms:
            df_t = df_t[df_t['Commodity'].isin(t_comms)]

        if df_t.empty:
            st.warning("Không có dữ liệu!")
        else:
            # Grouping
            df_plot = df_t.groupby(['Year', color_col])[
                'Value'].sum().reset_index()

            # Plot 1: Line Chart (Trend)
            st.plotly_chart(px.line(df_plot, x='Year', y='Value', color=color_col, markers=True,
                            title=f"Xu hướng {t_attr} qua các năm"), use_container_width=True)
            ...
            with col_a:
                ...
                # Area and Production for a Bubble Chart
                ...
                if 'Diện tích' in bubble_pivot.columns and 'Sản lượng' in bubble_pivot.columns:
                    ...
                    st.info(
                        "Cần có dữ liệu 'Diện tích' và 'Sản lượng' để hiển thị biểu đồ bong bóng.")
