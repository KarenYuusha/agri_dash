...
def show_geography_page():
    ...
    if df_geo is not None:
        ...
        with c1:
            sel_year = st.selectbox("Chọn Năm", sorted(df_geo['Year'].unique(), reverse=True), key="geo_year")
        with c2:
            sel_attr = st.selectbox("Chọn Chỉ số", sorted(df_geo['Attribute'].unique()), key="geo_attr")
        with c3:
            # CHANGED: Use multiselect instead of selectbox
            all_comms = sorted(df_geo['Commodity'].unique())
            sel_comms = st.multiselect("Chọn Nông sản (có thể chọn nhiều)", all_comms, default=[all_comms[0]], key="geo_comm")

        # --- XỬ LÝ DỮ LIỆU (Grouping) ---
        if not sel_comms:
            st.warning("Vui lòng chọn ít nhất một loại nông sản.")
        else:
            # Filter by Year, Attribute, and the list of Commodities
            ...
            # Group by Province to sum values if multiple commodities are selected
            # This ensures 1 column per province on the map
            ...
            if map_data.empty:
                ...
                map_data['elevation'] = map_data['Value'] / (max_val if max_val > 0 else 1) * 200000
                ...
                st.dataframe(map_data.nlargest(10, 'Value')[['Province', 'Value', 'Unit']], use_container_width=True)
